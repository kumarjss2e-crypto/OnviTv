...
const MainTabs = ({ navigation }) => {
  const { user } = useAuth();
  const { isFreeTier, loading } = useSubscription();
  const { showModal } = usePremiumUpgradeModal();

  useEffect(() => {
    console.log('[MainTabs] Component mounted/rendered');
  }, []);

  // Show modal when conditions are met
  useEffect(() => {
    console.log('[MainTabs] Checking if should show modal');
    
    if (loading) {
      console.log('[MainTabs] ❌ Subscription still loading');
      return;
    }

    if (!user?.uid) {
      console.log('[MainTabs] ❌ No user logged in');
      return;
    }

    if (!isFreeTier) {
      console.log('[MainTabs] ❌ User is premium');
      return;
    }

    console.log('[MainTabs] ✅ Showing premium modal');
    showModal(navigation);
  }, [loading, user?.uid, isFreeTier, showModal, navigation]);
